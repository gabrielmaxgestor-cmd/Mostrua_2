...
export default function ProductPage() {
  ...
  if (!reseller || !product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0A0A0F] text-center px-4">
        ...
        <p className="text-white/50 mb-6">Este produto pode ter sido removido ou está indisponível.</p>
        <button onClick={() => navigate(-1)} className="px-6 py-3 bg-[#13131C] border border-white/10 text-white rounded-xl font-medium mb-3 w-full max-w-xs flex items-center justify-center gap-2">
          <ArrowLeft className="w-5 h-5" /> Voltar
        </button>
        <Link to={storeUrl} className="px-6 py-3 bg-[#1A1A2E] text-white rounded-xl font-medium w-full max-w-xs flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] pb-24">
      {/* Header */}
      <header className="bg-[#13131C] sticky top-0 z-40 shadow-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(-1)} className="p-2 -ml-2 text-white/60 hover:bg-[#13131C] rounded-full transition-colors" title="Voltar">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <Link to={storeUrl} className="p-2 text-white/60 hover:bg-[#13131C] rounded-full transition-colors" title="Início da Loja">
              <Home className="w-5 h-5" />
            </Link>
          </div>
          <h1 className="font-bold text-lg text-white truncate max-w-[200px]">{reseller.storeName}</h1>
          <div className="flex items-center">
            <button onClick={() => setShowCart(true)} className="relative p-2.5 bg-[#0A0A0F] rounded-full hover:bg-[#13131C] transition-all">
      ...
      <div className="max-w-5xl mx-auto px-4 py-4">
        <div className="flex items-center text-sm text-white/50 overflow-x-auto hide-scrollbar whitespace-nowrap">
          <Link to={storeUrl} className="hover:text-white flex items-center gap-1">
      ...
      <main className="max-w-5xl mx-auto px-4">
        <div className="bg-[#13131C] rounded-3xl shadow-md border border-white/5 overflow-hidden flex flex-col md:flex-row">
          
          {/* Left Column - Gallery */}
          <div className="w-full md:w-1/2 p-4 md:p-6 flex flex-col gap-4">
            <div 
              className="relative aspect-square md:aspect-[4/5] rounded-2xl overflow-hidden bg-[#0A0A0F] group cursor-pointer"
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/0 md:group-hover:bg-black/5 transition-colors duration-300 flex items-center justify-center">
                <div className="bg-[#13131C]/90 backdrop-blur-sm text-white/90 p-3 rounded-full opacity-0 md:group-hover:opacity-100 transition-all duration-300 shadow-md transform scale-90 md:group-hover:scale-100">
                ...
                {product.images.map((img: string, idx: number) => (
                  <button 
                    ...
                    className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shrink-0 border-2 transition-all p-0.5 snap-start ${
                      currentImageIndex === idx ? 'border-orange-500 shadow-md ring-1 ring-orange-500' : 'border-transparent hover:border-white/20 opacity-60 hover:opacity-100'
                    }`}
                  >
                    <div className="w-full h-full rounded-lg overflow-hidden bg-[#0A0A0F]">
          ...
          <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col">
            <div className="flex items-start justify-between gap-4 mb-2">
              ...
              <button onClick={handleShare} className="p-2 text-white/40 hover:text-white bg-[#0A0A0F] hover:bg-[#13131C] rounded-full transition-colors shrink-0">
            ...
            <div className="mb-6 flex flex-col p-6 rounded-3xl bg-opacity-10" style={{ backgroundColor: `${reseller?.settings?.secondaryColor || '#e5e7eb'}20` }}>
              {formattedOriginalPrice && (
                <span className="text-lg md:text-xl text-white/50 line-through mb-1">
                  {formattedOriginalPrice}
                </span>
              )}
            ...
            {product.description && (
              <div className="mb-8">
                <p className={`text-white/60 leading-relaxed whitespace-pre-wrap ${!isDescExpanded && product.description.length > 200 ? 'line-clamp-3' : ''}`}>
                  {product.description}
                </p>
            ...
            <div className="mb-8 flex flex-col sm:flex-row sm:items-start gap-6">
              ...
              {product.variations && product.variations.length > 0 && (
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-bold text-white">Variação (Cor/Tamanho)</h3>
                    {!selectedVariation && <span className="text-xs text-red-500 font-medium">Obrigatório</span>}
                  </div>
                  
                  {product.variations.length > 4 ? (
                    <select
                      className="w-full px-4 py-3 rounded-xl border-2 border-white/10 bg-[#13131C] focus:ring-2 focus:ring-gray-900 outline-none text-white font-bold transition-all disabled:opacity-50"
                        ...
                        <button 
                          ...
                          className={`px-4 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${
                            selectedVariation === v 
                              ...
                              : isOutOfStock 
                                ? 'border-white/10 text-white/40 line-through opacity-50 cursor-not-allowed'
                                : 'border-white/10 text-white/70 hover:border-white/20 bg-[#13131C]'
                          }`}
              ...
              <div className="shrink-0 w-full sm:w-auto">
                <h3 className="font-bold text-white mb-3">Quantidade</h3>
                <div className="flex items-center gap-4">
                  <div className="flex items-center bg-[#0A0A0F] border border-white/10 rounded-xl p-1 w-full sm:w-auto justify-between sm:justify-start">
                    <button 
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      disabled={quantity <= 1 || isOutOfStock}
                      className="w-10 h-10 flex items-center justify-center text-white/60 hover:bg-[#13131C] hover:shadow-md rounded-lg transition-all disabled:opacity-50"
                    >
            ...
            {useStockControl && product.stock !== undefined && product.stock > 0 && (
                <div className="mb-6 -mt-4 text-sm text-white/50 font-medium">
      ...
      {showWaModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 overflow-y-auto">
          <div className="min-h-full flex items-center justify-center p-4 sm:p-6">
            <div className="bg-[#13131C] rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-full">
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#25D366]/10 shrink-0">
                ...
                <button onClick={() => setShowWaModal(false)} className="text-white/40 hover:text-white/60">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-white/10 focus:ring-2 focus:ring-[#25D366] outline-none"
                  placeholder="Como devemos te chamar?"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-white/70 mb-1">Seu WhatsApp</label>
                <input 
                  type="tel" 
                  required
                  value={waPhone}
              ...
              <div className="mt-2 shrink-0 border-t border-white/5 pt-4">
                <button 
                  type="submit"
        ...
        {isZoomModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 md:p-10 backdrop-blur-md">
            <button 
              ...
              className="absolute top-4 right-4 md:top-8 md:right-8 text-white/70 hover:text-white bg-[#13131C]/10 hover:bg-[#13131C]/20 p-3 rounded-full transition-colors z-50"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            >
              ...
              {product.images && product.images.length > 1 && (
                <>
                  <button 
                    className="absolute left-2 md:left-8 top-1/2 -translate-y-1/2 p-3 md:p-4 bg-[#13131C]/10 text-white rounded-full hover:bg-[#13131C]/20 transition-colors backdrop-blur-md"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <button 
                    className="absolute right-2 md:right-8 top-1/2 -translate-y-1/2 p-3 md:p-4 bg-[#13131C]/10 text-white rounded-full hover:bg-[#13131C]/20 transition-colors backdrop-blur-md"
                    onClick={(e) => {
                      e.stopPropagation();
