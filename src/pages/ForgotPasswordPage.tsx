  ...
  if (sent) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="max-w-md w-full text-center">
        ...
        <Link to="/login" className="text-orange-500 font-bold hover:underline">Voltar ao login</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <Link to="/login" className="flex items-center gap-2 text-white/50 hover:text-white/70 mb-8">
          <ArrowLeft className="w-4 h-4" /> Voltar ao login
        </Link>
        <h1 className="text-2xl font-bold text-white mb-2">Recuperar senha</h1>
        <p className="text-white/50 mb-6">Digite seu email e enviaremos um link de recuperação.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-white/70 mb-1">Email</label>
            <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
              placeholder="seu@email.com"
              className="w-full border-2 border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500" />
          </div>
