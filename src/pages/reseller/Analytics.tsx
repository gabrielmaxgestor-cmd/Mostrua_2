  ...
  if (!loading && stats.current7DaysViews === 0 && stats.totalOrders === 0) {
    return (
      <div className="text-center py-16">
        ...
        <h2 className="text-xl font-bold text-white">Ainda sem dados</h2>
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          ...
          <h3 className="text-white/50 text-sm font-medium">Visitas (7 dias)</h3>
          <p className="text-2xl font-bold text-white">{stats.current7DaysViews}</p>
          <p className="text-xs text-white/40 mt-1">vs {stats.previous7DaysViews} na semana anterior</p>
        </div>
        ...
        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          ...
          <h3 className="text-white/50 text-sm font-medium">Pedidos (7 dias)</h3>
        ...
        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          ...
          <h3 className="text-white/50 text-sm font-medium">Taxa de Conversão</h3>
          <p className="text-2xl font-bold text-white">{stats.conversionRate.toFixed(2)}%</p>
        </div>
        ...
        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          ...
          <h3 className="text-white/50 text-sm font-medium">Ticket Médio</h3>
          <p className="text-2xl font-bold text-white">R$ {stats.averageTicket.toFixed(2)}</p>
        </div>
      </div>

      {/* Chart */}
      <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5 overflow-x-auto">
        <h3 className="text-lg font-bold text-white mb-6">Visitas nos últimos 14 dias</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Eye className="w-5 h-5 text-white/40" /> Produtos Mais Visualizados
          </h3>
          <div className="space-y-4">
            {topViewedProducts.map((p, i) => (
              <div key={p.productId} className="flex items-center gap-4">
                <span className="text-lg font-bold text-white/30 w-4">{i + 1}</span>
                <div className="w-12 h-12 rounded-lg bg-[#13131C] overflow-hidden shrink-0">
                  {p.image ? <img src={p.image} alt={p.name} className="w-full h-full object-cover" /> : null}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate">{p.name}</p>
                  <p className="text-xs text-white/50">{p.views} visualizações</p>
                </div>
              </div>
            ))}
            {topViewedProducts.length === 0 && <p className="text-sm text-white/50 text-center py-4">Nenhum dado disponível</p>}
          </div>
        </div>

        <div className="bg-[#13131C] p-6 rounded-2xl shadow-sm border border-white/5">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <ShoppingCart className="w-5 h-5 text-white/40" /> Mais Adicionados ao Carrinho
          </h3>
          <div className="space-y-4">
            {topCartProducts.map((p, i) => (
              <div key={p.productId} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate">{p.name}</p>
                  <p className="text-xs text-white/50">{p.addedToCart || 0} adições</p>
                </div>
              </div>
            ))}
            {topCartProducts.length === 0 && <p className="text-sm text-white/50 text-center py-4">Nenhum dado disponível</p>}
          </div>
        </div>
      </div>
    </div>
